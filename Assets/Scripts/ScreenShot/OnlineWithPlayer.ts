import { Animator, AvatarIKGoal, GameObject, Quaternion, Transform, Vector3 } from 'UnityEngine';
...
export default class OnlineWithPlayer extends ZepetoScriptBehaviour {
    ...
    public SetPlayer(player : ZepetoPlayer, selfieStickPrefab : GameObject) {
        ...
        if(!player.isLocalPlayer){
            ...
            this.selfieStick = GameObject.Instantiate<GameObject>(selfieStickPrefab).transform
    ...
    public OnUpdateOnline(player : ZepetoPlayer, screenPos : Vector3){
        ...
        // 찍고 있는 플레이어 foreach
        const takingUser = ClientStarter.instance.GetRoom().State.selfiePlayer.get_Item(player.id)
        ...
        if(takingUser.cameraTransform == null || takingUser.cameraTransform == undefined){
            ...
            if(!player.isLocalPlayer){
                player.character.transform.LookAt(new Vector3(this.cameraTarget.position.x, player.character.transform.position.y, this.cameraTarget.position.z))
            }
        }
    }
